...
function getWeather() {
  fetch("http://api.openweathermap.org/data/2.5/weather?id=" + cityId + "&appid=b078a7d81849e7d2b796924e54583a40")
    ...
    .then(function (data) {
      ...
      document.querySelector('.clouds').innerHTML = `<span>${data.weather[0]['description']}</span>`;
      document.querySelector('.visibility').innerHTML = `Visibility:     <span>${data.visibility} m</span>`;
      ...
      document.querySelector('.preasure').innerHTML = `Preasure:<span>${data.main.pressure} hpa</span>`;
      document.querySelector('.humidity').innerHTML = `Humidity:<span>${data.main.humidity}&#37</span>`;
    })
    .catch(function () {
      // catch any errors
    });
