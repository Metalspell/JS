...
let ar1 = ['Norway', 4, true, 6, 'travel', false];
...
let ar2 = ['Norway', 4, true, 6, 'travel', false];
...
let ar3 = ['Norway', 4, true, 6, 'travel', false];
...
let ar6 = ['Dmitry', 'Capricorn', 2, 'January'];
...
function f9() {
  let out = ar9.length - 1;
  document.querySelector('.out-9').innerHTML = ar9[out];
}
...
function f10() {
  let lastElemIndex = ar10.length - 1
  let sum = ar10[1] + ar10[lastElemIndex];
...
function f12() {
  let out = '';
  let lastElem = ar12.length - 1;
  let buffer = ar12[lastElem];
  ar12[lastElem] = ar12[0];
  ar12[0] = buffer;
...
function f16() {
  ...
  for (let i = 0; i < ar16.length; i++) {
    if (ar16[i] % 2 === 0) {
      ar16_even[counter1] = `${ar16[i]} `;
      counter1++;
    } else {
      ar16_odd[counter2] = `${ar16[i]} `;
      counter2++;
    }
  }
