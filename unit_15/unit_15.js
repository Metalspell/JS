...
let s2 = new Set;
const f2 = () => {
  let data = document.querySelector('.i-2');
  s2.add(data.value);
  console.log(s2);
}
...
const f3 = () => {
  let data = document.querySelector('.i-3');
  let result = s3.has(data.value);
  if (result === true) {
    s3.delete(data.value);
  }
  console.log(s3);
}
...
let result = null;
const f4 = () => {
  let data = document.querySelector('.i-4');
  result = s4.has(data.value);
  if (result === true) {
...
const f7 = () => {
  let out;
  let data = document.querySelector('.i-7');
  let arr = data.value.split('');
  let s7 = new Set(arr);
  if (s7.size === arr.length && s7.size > 6) {
    out = 1;
  } else {
    out = 0;
  }
...
// Task 8
// При нажатии b-8 выполняете функцию f8. Функция должна перебрать набор s8 и добавить в массив ar8 только числа больше 5. 
...
// Task 9
//  При нажатии b-9 выполняете функцию f9. Функция должна принимать набор our_set в качестве параметра, 
...
const f13 = () => {
  let newArr = str13.split('');
  let s13 = new Set(newArr);
  let out = {};
  let counter = 0;
  for (i of s13) {
