...
function t4(e) {
  document.querySelector('.div-4').addEventListener('touchstart', myTouch);
}
...
function t5(e) {
  ...
  document.querySelector('.div-4').removeEventListener('touchstart', myTouch);
}
...
function t8() {
  let min = 0;
  let max = a8.length - 1;
  let arrIndex = Math.floor(Math.random() * (max - min + 1)) + min;
  console.log(1)
  this.style.background = a8[arrIndex];
}
...
function t11(e) {
  console.log(e)
  document.querySelector('.out-11').innerHTML += e.targetTouches[0].radiusX + '<br>';
  document.querySelector('.out-11').innerHTML += e.targetTouches[0].radiusY + '<br>';
}
...
let countOfElements = document.querySelectorAll('.img-12-min').length - 1;
let linkToImg = null;

function primaryImg() {
  linkToImg = images[count].getAttribute('src');
  document.querySelector('.img-12-max').setAttribute('src', linkToImg);
}

function nextFunction(e) {
  if (count == countOfElements) {
    count = 0;
    images[countOfElements].classList.remove('active-img');
  } else {
    count++;
